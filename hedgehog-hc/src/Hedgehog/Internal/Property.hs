...
--   /'Show' instance./
--
forAllWithT :: (Monad m, HasCallStack) => String -> (a -> String) -> GenT m a -> PropertyT m a
forAllWithT lbl render gen = do
  x <- PropertyT $ lift (Gen.label lbl gen)
...
--   /'Show' instance./
--
forAllWith :: (Monad m, HasCallStack) => String -> (a -> String) -> Gen a -> PropertyT m a
forAllWith lbl render gen =
  withFrozenCallStack $ forAllWithT lbl render $ Gen.generalize gen

-- | Generates a random input for the test by running the provided generator.
--
--
forAllT :: (Monad m, Show a, HasCallStack) => String -> GenT m a -> PropertyT m a
forAllT lbl gen =
  withFrozenCallStack $ forAllWithT lbl showPretty gen

-- | Generates a random input for the test by running the provided generator.
--
forAll :: (Monad m, Show a, HasCallStack) => String -> Gen a -> PropertyT m a
forAll lbl gen =
  withFrozenCallStack $ forAllWith lbl showPretty gen
