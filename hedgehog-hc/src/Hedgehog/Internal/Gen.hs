...
import           Control.Monad (MonadPlus(..), filterM, guard, join)
...
instance Monad m => Monad (GenT m) where
  ...
  (>>=) m k =
    GenT $ \size seed -> runGenT size seed . k =<< runGenT size seed m
...
embedGenT f gen =
  GenT $ \size seed ->
    (runGenT size seed . f) `embedTreeMaybeT`
...
instance MonadCatch m => MonadCatch (GenT m) where
    ...
    GenT $ \size seed ->
      (runGenT size seed m) `catch`
      (runGenT size seed . onErr)
...
instance MonadError e m => MonadError e (GenT m) where
  ...
  catchError m onErr =
    GenT $ \size seed ->
      (runGenT size seed m) `catchError`
      (runGenT size seed . onErr)
...
label :: MonadGen m => String -> m a -> m a
label lbl =
  withGenT $ \gen ->
    GenT $ \size seed ->
      runGenT size (Seed.selectBy lbl seed) gen
...
choice = withFrozenCallStack . \case
  ...
  xs -> do
    n <- label "n" $ integral $ Range.constant 0 (length xs - 1)
    label (show n) (xs !! n)
  ...
  let
    try k =
      if k > 100 then
        ...
        (x, gen) <- label "(x, gen)" $ freeze $ scale (2 * k +) gen0

        case p x of
          Just _ ->
            label "gen" $ withGenT (mapGenT (Tree.mapMaybeMaybeT p)) gen
          Nothing ->
            label "rec" $ try (k + 1)
  in
    try 0
  ...
  let
    try k =
      if k > 100 then
        ...
        case p x of
          Just _ ->
            label "gen" (withGenT (mapGenT (Tree.mapMaybeT p)) gen)
          Nothing ->
            label "try" (try (k + 1))
  in
    try 0
  ...
  in
    sized $ \size ->
      ...
      withGenT (mapGenT (TreeT . interleave . runTreeT)) $ do
        n <- label "n" (integral_ range)
        label "gen" (replicateM n (toTreeMaybeT gen))
...
map range gen =
  sized $ \size ->
    ...
    shrink Shrink.list $ do
      k <- label "k" $ integral_ range
      label "output" $ uniqueByKey k gen
  ...
  let
    try k xs0 =
      if k > 100 then
        ...
        label "gen" (replicateM n (freeze gen)) >>= \kvs ->
        case uniqueInsert n xs0 (fmap (first fst) kvs) of
          Left xs ->
            pure $ Map.elems xs
          Right xs ->
            label "rec" (try (k + 1) xs)
  in
    try (0 :: Int) Map.empty
...
shuffleSeq xs =
  if null xs then
    ...
    case Seq.lookup n xs of
      Just y ->
        (y Seq.<|) <$> label "shuffle" (shuffleSeq (Seq.deleteAt n xs))
      Nothing ->
    ...
    case Seq.splitAt n xs of
      (beginning, end) ->
        case Seq.viewl end of
          y Seq.:< end' ->
            (y Seq.<|) <$> label "shuffle" (shuffleSeq (beginning Seq.>< end'))
          Seq.EmptyL ->
            error "Hedgehog.Gen.shuffleSeq: internal error, lookup in empty sequence"
#endif
...
replicateM :: MonadGen m => Int -> m a -> m [a]
replicateM 0 _ = return []
replicateM n m = (:) <$> label "hd" m <*> label "tl" (replicateM (n - 1) m)
