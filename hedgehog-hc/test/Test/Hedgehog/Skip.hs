...
skipTestProperty logRef =
  withTests 5 . property $ do
    val@(curSize, _, shouldDiscard, shouldPass) <- forAll "val" $ do
      curSize <- Gen.sized pure
      let shouldDiscard = curSize == 1
      shouldPass <- Gen.shrink (\b -> [True | not b]) (pure $ curSize /= 2)
...
prop_SkipNothing =
  withTests 1 . property $ do
    ...
    case reportStatus report of
      Failed f -> do
        failureShrinks f === 3
        failureShrinkPath f === ShrinkPath [1, 0, 0]
...
prop_SkipToFailingShrink =
  withTests 1 . property $ do
    ...
    logs <- liftIO $ reverse <$> IORef.readIORef logRef
    logs === [(2, 3, False, False), (2, 2, False, False), (2, 0, False, False)]
...
prop_SkipToPassingShrink =
  withTests 1 . property $ do
    ...
    let
      prop =
        withSkip "3:a" $ skipTestProperty logRef

    report <- checkProp prop
    reportStatus report === OK

    logs <- liftIO $ reverse <$> IORef.readIORef logRef
    logs === [(2, 3, False, False), (2, 3, False, True)]
